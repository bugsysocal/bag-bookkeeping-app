...
**Project:** LedgerOne (placeholder) · **Covers:** Planning doc §4.3, §4.4, §11 items 4–5 · **Status:** APPROVED v1.1 (2026-07-03; #7 amended: write-off threshold **and** routing accounts are company-level settings; #2–5, #8–9 approved via scope audit — none touch VAT/WHT posting or the balance guarantee. **2026-07-06: added open Decision #11 — the Advisor-Mode-elevated bypass of the write-off limit is not yet decided; implementation deliberately left unbuilt pending it, see §7.5 and §9.**)
...
3. **Written off** — for genuinely unexplainable residuals: posts a real entry (two-line, balanced; deliberately **no VAT treatment** — no input-VAT back-out on written-off debits; advisor journals it if ever material). Targets and threshold are **company-level settings** (§8.3), seeded per below, advisor-editable per client (advisory practices flex these by client size — per the advisor-gated-controls rule these are Advisor Mode settings, seeded at company creation, not wizard questions for owners): debit lines → `writeoff_debit_account_id` (seeded 6980 Miscellaneous; fee-worded lines suggest 6900); credit lines → `writeoff_credit_account_id` (seeded 4200 Other Income). Guardrails: write-off above `writeoff_limit_kobo` (seeded ₦5,000) is **Advisor Mode only**; below it, owner may write off, staff never. Every write-off records the note thread in the entry memo and audit log. *(Decision #7, approved as amended)*

   ⏳ **Decision #11 (OPEN, added 2026-07-06) — what "Advisor Mode only" means for the above-limit case, precisely.** The current implementation (`ledger_core::recon::write_off`) is the conservative interim default: it refuses **any** write-off above `writeoff_limit_kobo` unconditionally, for every role — there is no elevated-bypass path yet, so "Advisor Mode only" above the limit is not actually wired to anything. Command-layer role gating (staff blocked entirely) shipped 2026-07-06 alongside the login/session work, but the amount-based elevation bypass itself is a business-logic decision, not merely a wiring task, and is being left unbuilt until decided. Open sub-questions for the next review: (a) once Advisor Mode is elevated, is an above-limit write-off allowed for **any** amount, or does a second, higher ceiling apply? (b) does it require anything beyond the existing mandatory note — e.g. re-entering the PIN at the moment of that specific write-off, rather than relying on an already-elevated session? (c) should an above-limit write-off be logged/surfaced differently to the owner than an ordinary one (e.g. its own banner-zone entry), given it bypasses a control the owner cannot see or edit? Until resolved, the hard refusal stands for all roles — safe, but stricter than "Advisor Mode only" as literally written above.
...
7. **Write-off targets and guardrail:** debits → 6980 (or 6900 if fee-worded), credits → 4200; owner may write off ≤ ₦5,000 (setting), above = Advisor PIN, staff never. Your call on the default threshold. (§7.5)
...
11. ⏳ **OPEN (added 2026-07-06) — the Advisor-Mode-elevated bypass of the write-off amount limit.** Not a wiring gap; a business-logic decision still to be made. See the callout under §7.5 path 3 for the exact question and the three sub-options. Interim implemented behavior: `write_off` refuses unconditionally above `writeoff_limit_kobo` regardless of role or elevation — safe, but not yet what "Advisor Mode only" implies. **Do not build the bypass until this is decided.**

---

*End of Spec 04. Next per §11 order: Spec 05/06 — Statements & reports (planning doc numbering merges §11 item 6). Decision #11 remains open and does not block Spec 05.*
